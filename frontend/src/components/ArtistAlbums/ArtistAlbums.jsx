...
const ArtistAlbums = ({ artistInfo, authToken }) => {
  ...
  async function getAlbums() {
    let response = await axios.get(
      `https://api.spotify.com/v1/artists/${artistInfo.id}/albums?include_groups=album&market=US&limit=30`,
      {
        headers: {
      ...
      <ImageList sx={{ width: 600, height: 450 }}>
        <ImageListItem key="SubHeader" cols={2}>
          <ListSubheader component="div">
            {artistInfo.name} Albums
          </ListSubheader>
        </ImageListItem>
        {albums.map((album) => (
          <ImageListItem key={album.images[0].url}>
            <img
              src={`${album.images[0].url}?w=248&fit=crop&auto=format`}
