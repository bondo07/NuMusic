...
const TopArtistsPage = ({user, token, authToken}) => {
    ...
    async function getTopArtists() {
        ...
        let top10Artists = newTopList.sort((a, b) => b.upvotes - a.upvotes).splice(0,10)
        await axios.get(`https://api.spotify.com/v1/artists?ids=${top10Artists.map((artist) => {
    ...
    return ( 
        <div>
            ...
            <Tabs variant="fullWidth" aria-label="Top Artists Tabs" value={value} onChange={handleChange}>
                {topVotedArtists.map((artist) => (
                    <Tab key={artist.id} label={artist.name} style={{color: 'white'}} icon={<img src={artist.images[2].url} alt="Artist"/>}/>
                ))}
            </Tabs>
            {topVotedArtists.map((artistInfo, i) => (
                <Panel value={value} index={i} key={artistInfo.id}>
                <Stack>
                    <h data-text={artistInfo.name} className="top-artist-name">{artistInfo.name}</h>
                    <div data-text={artistInfo.genres[0]} className="top-artist-genre">{artistInfo.genres[0]}</div>
                </Stack>
                    <Stack direction="row" style={{margin: "5rem"}} spacing={10}>
                        <Avatar alt={artistInfo.name} src={artistInfo.images[0].url} style={{width: "25rem", height: "25rem"}}/>
