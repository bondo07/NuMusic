...
const SearchResultsPage = ({
  ...
  async function getRelatedArtists() {
    let response = await axios.get(
      `https://api.spotify.com/v1/artists/${artistInfo.id}/related-artists/`,
      {
        headers: {
  ...
  return (
    <div>
      <Stack>
        <h6 data-text={artistInfo.name} className="artist-name">
          {artistInfo.name}
        </h6>
        <div data-text={artistInfo.genre} className="artist-genre">
          {artistInfo.genre}
        </div>
      </Stack>
